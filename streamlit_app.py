...
# ==================== STASIONERITAS DATA ====================
    ...
    # Add the code for stasioneritas here
Key Changes:
"Lanjutkan ke Data Preprocessing" Button in HOME Page: This button will set the menu to "DATA PREPROCESSING" and trigger a rerun of the app using st.experimental_rerun().

"Lanjutkan ke Stasioneritas Data" Button in DATA PREPROCESSING Page: The button will appear after the data preprocessing step is completed. Clicking it will set the menu to "STASIONERITAS DATA" and trigger the app to rerun.

Explicit Page Navigation: The session state (st.session_state.menu) is used to store the selected menu and dictate which page is displayed.

By adding buttons in each page and using st.session_state.menu, the page will update correctly without requiring complex navigation logic. This should resolve the issue of the button not showing up, as it is now explicitly tied to actions within the app.
...
Traceback:
File "/mount/src/penelitianku/streamlit_app.py", line 412, in <module>
    st.experimental_rerun()
    ^^^^^^^^^^^^^^^^^^^^^


ChatGPT said:
The error you're seeing is likely happening due to how st.experimental_rerun() is interacting with the session state or the page flow after the button is clicked. Instead of using st.experimental_rerun(), let's try an alternative approach by adjusting how the menu is handled and avoiding the rerun trigger.

Let me suggest an approach that focuses on controlling the flow directly through the session state and button actions without invoking st.experimental_rerun().

Revised Solution Without st.experimental_rerun()
Here's an updated approach that eliminates the use of st.experimental_rerun() and just relies on updating the session state directly:
...
# ==================== DATA PREPROCESSING ====================
elif st.session_state.menu == "DATA PREPROCESSING":
    st.markdown("<div class='header-container'>DATA PREPROCESSING</div>", unsafe_allow_html=True)
...
# ==================== STASIONERITAS DATA ====================
elif st.session_state.menu == "STASIONERITAS DATA":
    st.markdown("<div class='header-container'>STASIONERITAS DATA</div>", unsafe_allow_html=True)

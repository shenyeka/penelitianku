...
# ========================== #
   st.subheader("📘 PREDIKSI DATA TESTING")
        ...
        df_hybrid_test = pd.DataFrame({
            ...
            'Prediksi ANFIS ABC': forecast_anfis_denorm_trim,
            'Prediksi ARIMA-ANFIS ABC': pred_hybrid_test_abc
        })
        st.subheader("📊 Hasil Prediksi Gabungan ARIMA + ANFIS (ABC) - Testing")
        st.dataframe(df_hybrid_test)
        st.line_chart(df_hybrid_test.set_index('Tanggal')[['Prediksi ARIMA', 'Prediksi ARIMA-ANFIS ABC']])
        if 'actual_test' in st.session_state:
            aktual_test = st.session_state['actual_test'].reset_index(drop=True)[-min_len_test:].reset_index(drop=True)
            mape_test = np.mean(np.abs((aktual_test - pred_hybrid_test_abc) / aktual_test)) * 100
            st.success(f"📉 MAPE ARIMA-ANFIS (ABC) - Testing: {mape_test:.2f}%")
        else:
            st.info("Aktual test tidak tersedia untuk evaluasi MAPE.")
    except Exception as e:
        st.error(f"❌ Terjadi kesalahan saat proses pemodelan ARIMA-ANFIS ABC: {e}")

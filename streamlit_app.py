            ...
            if 'data_anfis' in st.session_state:
                ...
                if len(jp) > 1:
                    ...
                    if len(significant_lags) >= 2:
                        ...
                        st.subheader("Hasil Prediksi ANFIS")
                        st.line_chart({"Actual": target, "Predicted": predictions})

                    else:
                        st.warning("Jumlah lag signifikan kurang dari 2. Tidak cukup untuk input ANFIS.")
                else:
                    st.warning("Data terlalu sedikit untuk menghitung PACF.")
            else:
                st.warning("Data residual belum dinormalisasi.")
